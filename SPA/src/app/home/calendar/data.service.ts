...
export class DataService {
  ...
  getEvents(from: DayPilot.Date, to: DayPilot.Date): Observable<DayPilot.EventData[]> {
    // For now we return local mock events. In future switch to HTTP:
    // return this.http.get<DayPilot.EventData[]>(`/api/events?from=${from}&to=${to}`);
    return of(this.events as DayPilot.EventData[]);
  }
